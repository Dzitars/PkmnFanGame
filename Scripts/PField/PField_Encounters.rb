...
def pbGenerateWildPokemon(species,level,isroamer=false)
  ...
  # Custom Quest Code to determine if the spawning pokemon
  # should have increased odds
  #Kernel.pbMessage(genwildpoke.species.to_s)
  if genwildpoke.species == QUESTVARS[0]
    Kernel.pbMessage("Quest Pokemon")
    for i in 0...4   # 5 extra tries if on a quest
      break if genwildpoke.isShiny?
      #genwildpoke.makeShiny
      genwildpoke.personalID = rand(65536)|(rand(65536)<<16)
    end
  else
    #Kernel.pbMessage("Not Quest Pokemon")
  end
