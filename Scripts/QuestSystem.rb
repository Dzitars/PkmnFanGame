...
class Quest
  ...
  def initialize(id)
    @id         = id
    @stage      = 1
    @name       = $quest_data.getNameFromID(@id)
    @questDesc  = $quest_data.getQuestDescription(@id)
    @stageDesc  = $quest_data.getQuestStages(@id)[@stage-1]
  end
...
class Player_Quests
  ...
  def activateQuest(questID)
    ...
    for i in 0...@completed_quests.length
      if @completed_quests[i].id==questID
        #if quest is not repeatable, display error
        tmpRepeat = $quest_data.getQuestRepeat(questID)
        if(tmpRepeat<=0)
    ...
    #Kernel.pbMessage("Starting Quest")
    pbSEPlay("QuestStarted")
    
    #Kernel.pbMessage("QUESTVARS[0] = "+QUESTVARS[0].to_s)
    $game_variables[51] = QUESTVARS[0]
  ...
  def completeQuest(questID)
    ...
    for i in 0...@completed_quests.length
      if @completed_quests[i].id==questID
        #if quest is not repeatable, display error
        if(!$quest_data.getQuestRepeat(@active_quests[i].id))
          Kernel.pbMessage("You have already completed this quest")
          return
        else
          repeat = true
  ...
  def advanceQuestToStage(questID,stageNum)
    ...
    for i in 0...@active_quests.length
       if @active_quests[i].id==questID
         Kernel.pbMessage(@active_quests[i].stage.to_s)
  ...
  def getActiveQuestData
    return @active_quests.length > 0 ? @active_quests : []
    
    #questData = []
    #for i in 0...@active_quests.length
    #  questData.push(
    #    [@active_quests[i].name]
    #  )
    #end
...
#Test method to return the stages of events formated as a string
def getQuestStages(questID)
  stages = $quest_data.getQuestStages(questID)
  stageString = ""
  for i in 0...stages.length
    stageString += " "+stages[i]
  end
  
  return stageString
end

def getNameFromID(questID)
  return if !$quest_data
  return $quest_data.getNameFromID(questID)
end

def getQuestDescription(questID)
  return if !$quest_data
  #description = $quest_data.getQuestDescription(questID)
  
  return $quest_data.evalQuestDescription(questID)
  #return $quest_data.getQuestDescription(questID)
end

def isQuestActive(questID)
  return if !$quest_data
  return $PokemonGlobal.quests.isQuestActive(questID)
end

def isQuestAvailable(questID)
  return if !$quest_data
  return $PokemonGlobal.quests.isQuestAvailable(questID)
end

def getActiveQuestData
  return $PokemonGlobal.quests.getActiveQuestData
  #return $PokemonGlobal.quests.length>0 ? $PokemonGlobal.active_quests : []
end

def compileAllQuests
  $quest_data.compileAllQuests
...
module QuestsData
  ...
  NeededInfo={
     ...
     "FailedMessage"=>[26,"s"],
     "Repeat"=>[27,"i"],
     #These vars are optional and can be used to display custom script
     #in the QuestDescription
     "QuestVar0"=>[28,"s"],
     "QuestVar1"=>[29,"s"],
     "QuestVar2"=>[31,"s"],
#     "QuestVar3"=>[32,"s"],
...
class QuestInfo
  ...
  attr_reader :repeat
  attr_reader :questvars
  
  def initialize(id,questName,stages,rewardString,rewardDesc,
                  questDesc,locations,completedMessage,failedMessage,repeat,questvars)
#                  questDesc,locations,completedMessage,failedMessage,repeat)
    @id = id
    @name = questName
    @stages = stages
     ...
     if line[/^\s*\[\s*(\d+)\s*\]\s*$/]
       if currentQuest>0
         ...
         if repeat.nil? || repeat==""
           raise _INTL("Expected a repeat for quest {1}\r\n{2}",currentQuest,FileLineData.linereport)
         end
...
#                         questDesc,locations,completedMessage,failedMessage,repeat))
       ...
       if schema
         for i in 0...schemaValues.length
           ...
           when QuestsData::NeededInfo["FailedMessage"]
             ...
             break
           when QuestsData::NeededInfo['Repeat']
             repeat = record
             break
           #when matchData[1].include?("QuestVar")
           #  questvars.push(record)
           #  break
           else
             if matchData[1].include?("QuestVar")
                 questvars.push(record)
                 break
             elsif matchData[1].include?("StageLocation")
    ...
    @all_quests.push(QuestInfo.new(currentQuest,name,stages,rewardString,
      rewardDesc,questDesc,locations,completedMessage,failedMessage,repeat,questvars)) if lineCount>0
#      rewardDesc,questDesc,locations,completedMessage,failedMessage,repeat)) if lineCount>0
  ...
  def getQuestRepeat(questID)
    if questID.is_a?(String)
      questID = getIDFromName(questID)
    end
    for i in 0...@all_quests.length
      return @all_quests[i].repeat  if @all_quests[i].id==questID
    end
  end
  ...
  def evalQuestDescription(questID)
    #Evaluate the quest description to replace the contained variables
    questIndex = questID-1
    if @all_quests[questIndex].questvars.length>0
      #loop through each quest var
      for i in 0...@all_quests[questIndex].questvars.length
        #evaluate the quest variable, when formated correctly this
        #will set the global variable QUESTVARS[i]
        eval(@all_quests[questIndex].questvars[i])
        #if the quest description contains the proper formatting of ${i}
        if @all_quests[questIndex].questDesc.include? "${"+i.to_s+"}"
          #search and replace ${i} with the approprate QUESTVAR
          @all_quests[questIndex].questDesc.sub! "${"+i.to_s+"}", QUESTVARS[i].to_s
        else
          #raise _INTL("Expected $\{{1}\} in the quest description",i)
        end        
      end
    end
    
    return @all_quests[questIndex].questDesc
  end
...
class QuestScene 
  ...
  def pbStartScene
    ...
    @sprites["background"].setBitmap("Graphics/Pictures/Quest UI/quest_bg")
  ...
  def pbDrawNavigationInfo
    ...
    textPositions = [
      ["A,D - Change Section",2,Graphics.height-40,0,baseColor],
      ["V,B - Scroll Info",2,Graphics.height-20,0,baseColor],
      ["W.S - Change Quest",Graphics.width-170,Graphics.height-40,0,baseColor],
      ["X - Exit",Graphics.width-170,Graphics.height-20,0,baseColor],
    ]
   ...
   def pbDrawSections
    ...
    baseColor=Color.new(120,120,120)
    selectedColor=Color.new(0,0,0)
    shadowColor=Color.new(30,30,30)
    selectedShadowColor=Color.new(30,30,30)
    ...
    for i in 0...@sections.length
      ...
      if @sections[i][0]==$PokemonGlobal.selectedSection
        textPositions.push([@sections[i][1],85+(160*i),10,2,selectedColor])
      else
        textPositions.push([@sections[i][1],85+(160*i),10,2,baseColor])
      end
    end
  ...
  def getSelectedID
    #----------------------
    # Custom code block to ensure that program displays 
    # the correct starting section if no active quests are available
    #----------------------
    if($PokemonGlobal.selectedSection == 0 && $PokemonGlobal.quests.active_quests.length<=0)
      if($PokemonGlobal.selectedSection == 1 && $PokemonGlobal.quests.completed_quests.length<=0)
        $PokemonGlobal.selectedSection=2
      else
  ...
  def pbDrawQuestsList
    ...
    ret = getSelectedID
    quests = ret[0]
    #Kernel.pbMessage(ret.join(","))
    selectedID = ret[1]
    textPositions = []
    baseColor=Color.new(180,180,180)
    selectedColor=Color.new(255,255,255)
    shadowColor=Color.new(160,160,160)
    selectedShadowColor=Color.new(160,160,160)
    for i in 0...quests.length
      if quests[i].id==selectedID
        textPositions.push([$quest_data.getNameFromID(quests[i].id),0,@yOffsetList+2+(i*31),0,selectedColor])
      else
        textPositions.push([$quest_data.getNameFromID(quests[i].id),0,@yOffsetList+2+(i*31),0,baseColor])
      end
    end
    pbDrawTextPositions(overlay,textPositions)
    pbDrawQuestInfo
  ...
  def pbDrawQuestInfo
    ...
    selectedID = ret[1]
    textPositions = []
    baseColor=Color.new(175,175,175)
    shadowColor=Color.new(72,72,72)
    titleColor = Color.new(66,134,244)
    ...
    for i in 0...quests.length
      if quests[i].id==selectedID
        ...
        textPositions.push([$quest_data.getNameFromID(quests[i].id),(Graphics.width-200)/2,@yOffsetInfo,2,titleColor])
        @curNumLines+=1
        for j in 0...questDescArr.length
          textPositions.push([_INTL("{1}",questDescArr[j]),0,@yOffsetInfo+20+20*j,0,baseColor])
          @curNumLines+=1
        end
         #What's draw depends on if completed, failed, or active
        if $PokemonGlobal.selectedSection==2
          textPositions.push([_INTL("Result:"),0,@yOffsetInfo+20+20*questDescArr.length,0,selectedColor])
          textPositions.push([_INTL(" {1}",$quest_data.getFailedMessage(quests[i].id)),0,@yOffsetInfo+40+20*questDescArr.length,0,baseColor])
          @curNumLines+=1
          break
        elsif $PokemonGlobal.selectedSection==1 
          textPositions.push([_INTL("Result:"),0,@yOffsetInfo+20+20*questDescArr.length,0,selectedColor])
          textPositions.push([_INTL(" {1}",$quest_data.getCompletedMessage(quests[i].id)),0,@yOffsetInfo+40+20*questDescArr.length,0,baseColor])
          @curNumLines+=1
          break       
        else
          textPositions.push([_INTL("Reward:"),0,@yOffsetInfo+20+20*questDescArr.length,0,selectedColor])
          textPositions.push([_INTL("{1}",$quest_data.getQuestRewardDescription(quests[i].id)),65,@yOffsetInfo+20+20*questDescArr.length,0,baseColor])
          @curNumLines+=1
          textPositions.push([_INTL("Next Task:"),0,@yOffsetInfo+20+20*(questDescArr.length+1),0,selectedColor])
          @curNumLines+=1
          textPositions.push([_INTL(" {1}",stageDescArr[0]),0,@yOffsetInfo+40+20*(questDescArr.length+1),0,baseColor])
          @curNumLines+=1
          for k in 1...stageDescArr.length
            textPositions.push([_INTL("{1}",stageDescArr[k]),0,@yOffsetInfo+40+20*(questDescArr.length+1)+20*k,0,baseColor])
            @curNumLines+=1
          end
          ...
          textPositions.push([_INTL("Next Location:"),0,@yOffsetInfo+40+20*(questDescArr.length)+20*(stageDescArr.length+1),0,selectedColor])
          @curNumLines+=1
          textPositions.push([_INTL(" {1}",locationText),0,@yOffsetInfo+60+20*(questDescArr.length)+20*(stageDescArr.length+1),0,baseColor])
          @curNumLines+=1
          break
        end
      end
  ...
  def pbMain
    loop do
      ...
      elsif Input.trigger?(Input::R)  #up
        if (@yOffsetInfo/-20)<(@curNumLines-15)
          @yOffsetInfo-=20
          pbDrawQuestInfo
        end
      elsif Input.trigger?(Input::L) #down
        if @yOffsetInfo<0
          @yOffsetInfo+=20
          pbDrawQuestInfo
        end
