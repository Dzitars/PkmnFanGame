...
class PokemonPauseMenu_Scene
  ...
  def pbStartScene
    ...
    staticOptions = []
    #content.push(_INTL("X - Exit"))
    staticOptions.push([_INTL("{1} - Save","V"),16, 330, 0, Color.new(255,255,255),Color.new(0,0,0,65)])
    @sprites["textOverlay"].bitmap.blt(-2,362,bmp,Rect.new(0,0,bmp.width,bmp.height))
    @sprites["textOverlay"].bitmap.blt(-2,320,bmp,Rect.new(0,0,bmp.width,bmp.height))
    pbDrawTextPositions(@sprites["textOverlay"].bitmap,staticOptions)
...
class PokemonPauseMenu
  ...
  def pbStartPokemonMenu
    ...
    loop do
      ...
      elsif Input.trigger?(Input::L) #save hotkey
...
# Quests
MenuHandlers.addEntry(:QUESTS,_INTL("Quests"),"menuQuest",proc{|menu|
  if !hasAnyQuests?
