...
class PokemonSummary_Scene
  ...
  def drawPage(page)
    ...
    textpos = [
       ...
       ["Lvl.",5,92,0,Color.new(240,240,240)],
       [@pokemon.level.to_s,46,92,0,Color.new(248,248,248)],
       [_INTL("Item"),66,318,0,base,shadow]
    ]
    # Write the held item's name
    if @pokemon.hasItem?
      textpos.push([PBItems.getName(@pokemon.item),16,352,0,Color.new(64,64,64)])
    else
      textpos.push([_INTL("None"),16,352,0,Color.new(192,200,208)])
    end
  ...
  def drawPageOne
    overlay = @sprites["overlay"].bitmap
    base   = Color.new(248,248,248)
    base2  = Color.new(192,200,208)
    shadow = Color.new(70,70,70)
    dexNumBase   = (@pokemon.isShiny?) ? Color.new(248,56,32) : base2
    dexNumShadow = (@pokemon.isShiny?) ? Color.new(224,152,144) : Color.new(70,70,70)
    ...
    textpos = [
       ...
       [PBSpecies.getName(@pokemon.species),435,112,2,base2,shadow],
    ...
    if @pokemon.ot==""
      textpos.push([_INTL("RENTAL"),435,176,2,base2,shadow])
      textpos.push(["?????",435,208,2,base2,shadow])
    else
      ownerbase   = base2
      ownershadow = Color.new(176,176,176)
      case @pokemon.otgender
      when 0; ownerbase = Color.new(24,112,216); ownershadow = Color.new(76,108,148)
      when 1; ownerbase = Color.new(248,56,32);  ownershadow = Color.new(224,152,144)
      end
      textpos.push([@pokemon.ot,435,176,2,ownerbase,shadow])
      #textpos.push([sprintf("%05d",@pokemon.publicID),238,208,1])
    end
    ...
    if (@pokemon.isShadow? rescue false)
      textpos.push([_INTL("Heart Gauge"),238,240,0,base])
      ...
      textpos.push([_INTL("Exp. Points"),238,208,0,base,shadow])
      textpos.push([pbCommaNumber(@pokemon.exp),488,240,1,base2,shadow])
      textpos.push([_INTL("To Next Lv."),238,272,0,base,shadow])
      textpos.push([pbCommaNumber(endexp-@pokemon.exp),488,304,1,base2,shadow])
    end
    # Draw all text
    pbDrawTextPositions(overlay,textpos)
    # Draw Pokémon type(s)
  ...
  def drawPageTwo
    ...
    if shownature
      ...
      memo += _INTL("<c3=F83820,464646>{1}<c3=C0C8D0,464646> nature.\n",naturename)
    end
    # Write date received
    if @pokemon.timeReceived
      date  = @pokemon.timeReceived.day
      month = pbGetMonthName(@pokemon.timeReceived.mon)
      year  = @pokemon.timeReceived.year
      memo += _INTL("<c3=C0C8D0,464646>{1} {2}, {3}\n",date,month,year)
    end
    ...
    memo += sprintf("<c3=F83820,464646>%s\n",mapname)
    ...
    memo += sprintf("<c3=C0C8D0,464646>%s\n",mettext) if mettext && mettext!=""
    ...
    if @pokemon.obtainMode==1
      ...
      memo += _INTL("<c3=C0C8D0,464646>Egg hatched.\n")
    else
      memo += "\n" # Empty line
    end
  ...
  def drawPageThree
    ...
    base   = Color.new(248,248,248)
    shadow = Color.new(70,70,70)
    descColor = Color.new(192,200,208)
    ...
    textpos = [
       ...
       [sprintf("%d/%d",@pokemon.hp,@pokemon.totalhp),462,76,1,descColor,shadow],
       [_INTL("Attack"),248,120,0,base,statshadows[0]],
       [sprintf("%d",@pokemon.attack),456,120,1,descColor,shadow],
       [_INTL("Defense"),248,152,0,base,statshadows[1]],
       [sprintf("%d",@pokemon.defense),456,152,1,descColor,shadow],
       [_INTL("Sp. Atk"),248,184,0,base,statshadows[3]],
       [sprintf("%d",@pokemon.spatk),456,184,1,descColor,shadow],
       [_INTL("Sp. Def"),248,216,0,base,statshadows[4]],
       [sprintf("%d",@pokemon.spdef),456,216,1,descColor,shadow],
       [_INTL("Speed"),248,248,0,base,statshadows[2]],
       [sprintf("%d",@pokemon.speed),456,248,1,descColor,shadow],
       [_INTL("Ability"),224,284,0,base,shadow],
       [PBAbilities.getName(@pokemon.ability),362,284,0,descColor,shadow],
    ]
    # Draw all text
    pbDrawTextPositions(overlay,textpos)
    # Draw ability description
    abilitydesc = pbGetMessage(MessageTypes::AbilityDescs,@pokemon.ability)
    drawTextEx(overlay,224,316,282,2,abilitydesc,descColor,shadow)
    # Draw HP bar
    if @pokemon.hp>0
      hpzone = 0
  ...
  def drawPageFour
    ...
    # Write various bits of text
    textpos = [
       [_INTL("IV"),420,60,1,shadow,base],
       [_INTL("EV"),440,60,0,shadow,base],
       [_INTL("HP"),248,88,0,base,shadow],
       [sprintf("%d",@pokemon.iv[0]),420,88,1,descColor,shadow],
       [sprintf("%d",@pokemon.ev[0]),440,88,0,descColor,shadow],
       [_INTL("Attack"),248,120,0,base,statshadows[0]],
       [sprintf("%d",@pokemon.iv[1]),420,120,1,descColor,shadow],
       [sprintf("%d",@pokemon.ev[1]),440,120,0,descColor,shadow],
       [_INTL("Defense"),248,152,0,base,statshadows[1]],
       [sprintf("%d",@pokemon.iv[2]),420,152,1,descColor,shadow],
       [sprintf("%d",@pokemon.ev[2]),440,152,0,descColor,shadow],
       [_INTL("Sp. Atk"),248,184,0,base,statshadows[3]],
       [sprintf("%d",@pokemon.iv[3]),420,184,1,descColor,shadow],
       [sprintf("%d",@pokemon.ev[3]),440,184,0,descColor,shadow],
       [_INTL("Sp. Def"),248,216,0,base,statshadows[4]],
       [sprintf("%d",@pokemon.iv[4]),420,216,1,descColor,shadow],
       [sprintf("%d",@pokemon.ev[4]),440,216,0,descColor,shadow],
       [_INTL("Speed"),248,248,0,base,statshadows[2]],
       [sprintf("%d",@pokemon.iv[5]),420,248,1,descColor,shadow],
       [sprintf("%d",@pokemon.ev[5]),440,248,0,descColor,shadow],
       [_INTL("Ability"),224,284,0,base,shadow],
       [PBAbilities.getName(@pokemon.ability),362,284,0,descColor,shadow],
    ]
    # Draw all text
    ...
    # Draw ability description
    abilitydesc = pbGetMessage(MessageTypes::AbilityDescs,@pokemon.ability)
    drawTextEx(overlay,224,316,282,2,abilitydesc,descColor,Color.new(70,70,70))
  ...
  def drawPageSix
    ...
    # Write various bits of text
    textpos = [
       [_INTL("No. of Ribbons:"),234,332,0,base,shadow],
       [@pokemon.ribbonCount.to_s,450,332,1,base,shadow],
    ]
    # Draw all text
    pbDrawTextPositions(overlay,textpos)
    # Show all ribbons
