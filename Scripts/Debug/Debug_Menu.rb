...
def pbDebugMenuCommands(showall=true)
  ...
  #-------Custom options
  commands.add("main","questmenu",_INTL("Quest Options"),
    _INTL("Edit Quest Metadata"))
  commands.add("questmenu","resetquests",_INTL("Reset Quests"),
    _INTL("Reset all quest metadata"))
...
def pbDebugMenuActions(cmd="",sprites=nil,viewport=nil)
  ...
  when "resetquests"
    msgwindow=Kernel.pbCreateMessageWindow
    $PokemonGlobal.quests.active_quests.clear
    $PokemonGlobal.quests.completed_quests.clear
    $PokemonGlobal.quests.failed_quests.clear
    
    Kernel.pbMessageDisplay(msgwindow,_INTL("All quests have been reset."))
    Kernel.pbDisposeMessageWindow(msgwindow)
  end
  return false
end
